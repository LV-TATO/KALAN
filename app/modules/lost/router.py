...
def list_lost(skip: int = 0, limit: int = 20, zona: str | None = None, especie: str | None = None, db: Session = Depends(get_db)):
    return LostService(db).list_lost(skip=skip, limit=limit, zona=zona, especie=especie)

@router.get("/{perdida_id}", response_model=PerdidaOut)
def get_lost(perdida_id: int, db: Session = Depends(get_db)):
...
def create_lost(data: PerdidaCreate, usuario: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    return LostService(db).create_lost(data, usuario_id=usuario.id)
...
def update_lost(perdida_id: int, data: PerdidaUpdate, usuario: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    return LostService(db).update_estado(perdida_id, data.estado, usuario_id=usuario.id)

router.get("{perdida_id}/avistamientos", response_model=list[AvistamientoOut])
def get_sightings(perdida_id: int, db: Session = Depends(get_db)):
...
def create_sighting(perdida_id: int, data: AvistamientoCreate, usuario: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    return LostService(db).create_sighting(perdida_id, data, usuario_id=usuario.id)